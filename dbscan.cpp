...
DBSCAN::DBSCAN(const std :: vector<std :: vector<float>>& InputData,
               QWidget *parent, std :: vector<float>* const group,
               QMap<float, QString>* const map, std :: vector<float>* const ConstGroup) : QWidget(parent)
{
    setFixedSize(1400, 520);
    ...
    QLabel* tip = new QLabel("注：红色点为噪声点", this);
    tip -> setGeometry(50, 490, 200, 30);
